...
import { normalizeName, nameMatchesSearchTerms } from '@/utils/nameMatching';
  ...
  try {
    if (isPhoneSearch) {
      ...
      const matchedUsers = users.filter(user => {
        ...
        return userNormalizedPhone.includes(normalizedPhone) || normalizedPhone.includes(userNormalizedPhone);
      });
      ...
      const matchedAuthUsers = authUsers?.users?.filter(au => 
