 ...
 */
export function namesAreSimilar(name1: string, name2: string, threshold: number = 0.8): boolean {
 ...
 */
export function nameMatchesSearchTerms(name: string, searchQuery: string): boolean {
  const normalizedName = normalizeName(name);
  const searchTerms = normalizeName(searchQuery).split(' ').filter(term => term.length > 0);
