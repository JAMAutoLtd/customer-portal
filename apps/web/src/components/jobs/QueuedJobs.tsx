...
export function QueuedJobs() {
  ...
  const renderExpandedContent = (job: TechnicianJob) => {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <p className="text-sm text-gray-500">Assign Technician:</p>
          <select
            defaultValue={job.assigned_technician}
            className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            onChange={(e) =>
              handleReassignJob(job.id, parseInt(e.target.value))
            }
          >
