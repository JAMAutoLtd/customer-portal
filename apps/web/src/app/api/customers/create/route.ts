...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Create admin client for both duplicate check and user creation
    ...
    // Check for existing user with same email using admin client
    const { data: existingUsers, error: userCheckError } =
      await adminSupabase.auth.admin.listUsers({
        page: 1,
        perPage: 1000, // Set high limit to check more users
      })
    ...
    // Create auth user with temporary password (reusing admin client)
