...
# Imports from scheduler source
from src.scheduler.models import (
    Address, Technician, Job, SchedulableUnit, DailyAvailability, JobStatus,
    Order, CustomerType, CustomerVehicle, Service, ServiceCategory
)
from src.scheduler.scheduler import update_job_queues_and_routes, MAX_PLANNING_DAYS
from src.scheduler.utils import group_jobs_by_order, create_schedulable_units
from src.scheduler.availability import get_technician_availability
from src.scheduler.routing import fetch_distance_matrix, update_etas_for_schedule

# Imports from other test files (if needed for fixtures/mocks)
# Use relative imports now that __init__.py exists
from .test_scheduler import MockJob, MockAddress, MockTechnician, MockSchedulableUnit, mock_get_availability_update
...
DAY_START_INTEG = datetime(2024, 5, 20, 9, 0, 0) # Example date
...
def integ_technician():
    ...
    tech = MockTechnician(
        id=10, 
        name_placeholder="IntegTech", # Use placeholder name arg
        mock_van_equipment=["tool_a", "tool_b"], # Use correct arg name
...
def mock_final_eta_update():
    ...
    # If it calls something else, adjust the patch target
    with patch('src.scheduler.scheduler.update_job_etas') as mock_update:
        yield mock_update
